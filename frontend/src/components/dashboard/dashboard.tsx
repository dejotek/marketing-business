...
function formatLog(l:any) {
  ...
  switch(t) {
    case 'exam.submitted':
      return `Egzamin: kurs ${payload.courseId || ''}, moduł ${payload.moduleId || ''} — wynik ${payload.score || 0}/${payload.total || 0} ${payload.score && payload.total ? ((payload.score / Math.max(1,payload.total)) >= 0.6 ? '(Zaliczone)' : '(Niezaliczone)') : ''}`;
    case 'purchase':
